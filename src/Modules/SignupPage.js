...
const SignupPage = () => {
  ...
  const sendDataIntoDb = async () => {
    ...
    //   `/http://localhost:4000/addUser?email=${stateData?.["email"]}?password=${stateData?.["password"]}`
    // );
    // if (response.data.Status_Cd === 0) {
    //   dispatch(showSnackbar(true, "error", "Email exist"));
    //   return;
    // }
  ...
  const { handleSubmit, errors } = useFormik({
    ...
    onSubmit: async () => await sendDataIntoDb,
  });
